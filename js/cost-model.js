...
const cmApp = {
    ...
    async loadProjectData(projectId) {
        try {
            ...
            // Map DB column names to CM UI field names and split direct/indirect
            var directLabor = [];
            var indirectLabor = [];
            labor.forEach(function(l) {
                if (l.role_name && !l.activity_name) l.activity_name = l.role_name;
                if (l.annual_volume != null && l.volume == null) l.volume = parseFloat(l.annual_volume) || 0;
                if (l.calculated_uph != null && l.base_uph == null) l.base_uph = parseFloat(l.calculated_uph) || 0;
                if (l.role_category === 'indirect') {
                    if (!l.role_name) l.role_name = l.activity_name || '';
                    indirectLabor.push(l);
                } else {
                    directLabor.push(l);
                }
            });
    ...
    async saveProject() {
        try {
            ...
            try {
                await Promise.all([
                    persistChildTable('cost_model_labor', (this.projectData.laborLines || []).concat(this.projectData.indirectLaborLines || [])),
